...
from conteneurs.forms import DossiersForm, DossiersliberiaForm
...
class DossierViewLiberia(ListView):
    model = Dossier
    context_object_name = 'Dossier_list_liberia'
    template_name = 'pages/Dossier/Dossier_list_liberia.html'
    ...
    def get_queryset(self):
        ...
        pays = Pays.objects.filter(nom='liberia').first()  # Récupérer l'objet Pays pour 'liberia'
        if pays:
...
@method_decorator(login_required, name='dispatch')
class DossierCreateLiberiaView(CreateView):
    model = Dossier
    form_class = DossiersliberiaForm
    template_name = "pages/Dossier/Dossier_create_liberia.html"
    success_url = reverse_lazy('Dossier_list_liberia')
    ...
    def form_valid(self, form):
        ...
        pays_liberia = Pays.objects.get(nom="liberia")  # Remplacer par le pays souhaité
        form.instance.pays = pays_liberia
...
@method_decorator(login_required, name='dispatch')
class DossierUpdateLiberiaView(UpdateView):
    model = Dossier
    form_class = DossiersliberiaForm
    template_name = "pages/Dossier/Dossier_create_liberia.html"
    success_url = reverse_lazy('Dossier_list_liberia')
...
@method_decorator(login_required, name='dispatch')
class DossierDeleteLiberiaView(DeleteView):
    model = Dossier
    template_name = "pages/Dossier/Dossier_confirm_delete_liberia.html"
    success_url = reverse_lazy('Dossier_list_liberia')
...
def dossier_detail_liberia(request, pk):
    dossier = get_object_or_404(Dossier, pk=pk)
    return render(request, 'pages/Dossier/Dossier_detail_modal_liberia.html', {'dossier': dossier})
