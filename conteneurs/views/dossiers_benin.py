...
from conteneurs.forms import DossiersForm, DossiersBeninForm
...
class DossierViewBenin(ListView):
    model = Dossier
    context_object_name = 'Dossier_list'
    template_name = 'pages/Dossier/Dossier_list_Benin.html'
    ...
    def get_queryset(self):
        ...
        pays = Pays.objects.filter(nom='Benin').first()  # Récupérer l'objet Pays pour 'Benin'
        if pays:
...
@method_decorator(login_required, name='dispatch')
class DossierCreateBeninView(CreateView):
    model = Dossier
    form_class = DossiersBeninForm
    template_name = "pages/Dossier/Dossier_create_Benin.html"
    success_url = reverse_lazy('Dossier_list_benin')
    ...
    def form_valid(self, form):
        ...
        pays_benin = Pays.objects.get(nom="Benin")  # Remplacer par le pays souhaité
        form.instance.pays = pays_benin
...
@method_decorator(login_required, name='dispatch')
class DossierUpdateBeninView(UpdateView):
    model = Dossier
    form_class = DossiersBeninForm
    template_name = "pages/Dossier/Dossier_create_Benin.html"
    success_url = reverse_lazy('Dossier_list_benin')
...
@method_decorator(login_required, name='dispatch')
class DossierDeleteBeninView(DeleteView):
    model = Dossier
    template_name = "pages/Dossier/Dossier_confirm_delete_Benin.html"
    success_url = reverse_lazy('Dossier_list_benin')
...
def dossier_detail_Benin(request, pk):
    dossier = get_object_or_404(Dossier, pk=pk)
    return render(request, 'pages/Dossier/Dossier_detail_modal_Benin.html', {'dossier': dossier})
