...
class DossierViewGabon(ListView):
    model = Dossier
    context_object_name = 'Dossier_list'
    template_name = 'pages/Dossier/Dossier_list_gabon.html'
    ...
    def get_queryset(self):
        ...
        pays = Pays.objects.filter(nom='Gabon').first()  # Récupérer l'objet Pays pour 'Gabon'
        if pays:
...
@method_decorator(login_required, name='dispatch')
class DossierCreateGabonView(CreateView):
    model = Dossier
    form_class = DossiersGabonForm
    template_name = "pages/Dossier/Dossier_create_gabon.html"
    success_url = reverse_lazy('Dossier_list_gabon')
    ...
    def form_valid(self, form):
        ...
        pays_cote_ivoire = Pays.objects.get(nom="Gabon")  # Remplacez par le pays souhaité
...
@method_decorator(login_required, name='dispatch')
class DossierUpdateGabonView(UpdateView):
    model = Dossier
    form_class = DossiersGabonForm
    template_name = "pages/Dossier/Dossier_create_gabon.html"
    success_url = reverse_lazy('Dossier_list_gabon')
...
@method_decorator(login_required, name='dispatch')
class DossierDeleteGabonView( DeleteView):
    model = Dossier
    template_name = "pages/Dossier/Dossier_confirm_delete_gabon.html"
    success_url = reverse_lazy('Dossier_list_gabon')
...
def dossier_detail_gabon(request, pk):
    dossier = get_object_or_404(Dossier, pk=pk)
    return render(request, 'pages/Dossier/Dossier_detail_modal_gabon.html', {'dossier': dossier})